...
class RootWidget(GridLayout):
    ...
    def update_Spectrogram_Image(self, image):
        ...
        # arr = np.ndarray(shape=[256, 512, 3], dtype=np.uint8)
        # convert it to texture
        # buf1 = cv2.flip(spectrogram, 0)
        # buf = buf1.tostring()
        # initialize the array with the buffer values
...
class ThreadedApp(App):
    ...
    def __init__(self, **kwargs):
        super(ThreadedApp, self).__init__(**kwargs)
